...
contract WorkerDescriptionRegistry is Pausable {
   ...
   */
  function addWorkerDescription(string ipfsURI) public whenNotPaused {
   ...
   * @param ipfsURI - SHA2-256 IPFS hash for worker description json
   */
  function updateWorkerDescription(uint256 id, string ipfsURI) public onlyWorkerDescriptionOwner(id) whenNotPaused {
    require(bytes(ipfsURI).length != 0);
   ...
   */
  function getWorkerOwnerAddresses() public view returns (address[]) {
